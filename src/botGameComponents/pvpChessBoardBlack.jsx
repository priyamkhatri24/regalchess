...
class PvpChessBoardBlack extends Component {
  state = {};
  constructor(props) {
    super(props);
    this.state = {
    ...
    if (
      ...
      alert(`black has been check Mated`);
    } else if (
      this.state.game.check === true &&
      this.state.game.turn === "black"
    ) {
      alert(`black has been given check`);
    }
  }
  ...
  componentDidUpdate = async (prevProps) => {
    if (this.props !== prevProps) {
      ...
      if (this.state.game.turn === "white") {
        setTimeout(() => {
          this.props.moveWhite();
        }, 500);
      }
    }
  };
  checkIfBlackColourSelected = (piece) => {
    if (
  ...
  selectPiece = async (e, squareId) => {
    if (
      this.state.game.turn === "black" &&
      this.checkIfBlackColourSelected(this.state.game.pieces[squareId])
    ) {
      await this.setState({
        fromSelected: true,
        from: squareId,
  ...
  pushToParent = async () => {
    if (this.state.fromSelected && this.state.toSelected) {
      await this.props.moveBlackFromTo(this.state.from, this.state.to);
      await this.setState({
        fromSelected: false,
        toSelected: false,
        from: "",
  ...
  render() {
    return (
      <div className="board-deasign black-board">
...
export default PvpChessBoardBlack;
